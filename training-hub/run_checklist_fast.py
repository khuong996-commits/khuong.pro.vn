...
def update_firestore_daily(token, date_str, email, data):
    ...
    payload = {
        "fields": {
            ...
            "date": date_str,
            "fb_posts": fb_posts,
            "zalo_posts": zalo_posts,
            "new_customers": new_customers,
            "viewing_customers": viewing_customers,
            "notes": notes,
            "updated_by": "auto_check_script_fast",
            "updated_at": datetime.utcnow().isoformat() + "Z"
        }
    }
    
    # Format fields for Firestore v1
    firestore_payload = {
        "fields": {
            k: {"stringValue": str(v)} if isinstance(v, (int, float, str)) else v 
            for k, v in payload["fields"].items()
        }
    }
    # Fix the types correctly
    firestore_payload["fields"]["fb_posts"] = {"integerValue": str(fb_posts)}
    firestore_payload["fields"]["zalo_posts"] = {"integerValue": str(zalo_posts)}
    firestore_payload["fields"]["new_customers"] = {"integerValue": str(new_customers)}
    firestore_payload["fields"]["viewing_customers"] = {"integerValue": str(viewing_customers)}
    firestore_payload["fields"]["updated_at"] = {"timestampValue": payload["fields"]["updated_at"]}
    
    patch_res = requests.patch(url, headers=headers, json=firestore_payload)
...
def main():
    ...
    # By default, check only Today and Yesterday for maximum speed
    # If "--all" argument is passed, check the whole week
    check_dates = [base_date - timedelta(days=1), base_date]
    if "--all" in sys.argv:
        current_weekday = base_date.weekday()
        start_of_week = base_date - timedelta(days=current_weekday)
        check_dates = [(start_of_week + timedelta(days=i)) for i in range(7)]
        print("Mode: Full Week check.")
    else:
        print("Mode: Fast Check (Yesterday & Today only). Use --all for full week.")
    ...
    # Run Sheet reads in parallel threads
    sheet_data_by_day = {}
    ...
    print("\nChecklist fast sync completed successfully!")
